...
export default function Signup()
{
    ...
    const handlesignin = async(e)=>{
        ...
        try{
            ...
            if(pass === passc && terms)
                {
                    const result = await axios.post(`${process.env.REACT_APP_API_URL}/signup`, formdata);
    ...
    return(
        <div className="h-full bg-black">
            <div class="flex min-h-full flex-col items-center justify-center px-6 py-12 lg:px-8">
                <div className="flex min-h-full flex-col justify-center px-8 py-12 lg:px-8 w-full lg:w-1/2 bg-gray-900 border-2 border-gray-500 rounded">
                <div class="sm:mx-auto sm:w-full sm:max-w-sm">
                    <img src="./assets/ai_logo.png" alt="Your Company" class="mx-auto h-20 w-auto" />
                    <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-white">Set up your account</h2>
                </div>
                ...
                <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                    ...
                    <div>
                        ...
                        <label for="password" class="block text-sm/6 font-medium text-gray-100">Setup a Profile Picture</label>
                        </div>
                        <div class="mt-2">
                        <input id="picture" onChange={(e)=>setprofilePic(e.target.files[0])} type="file" name="picture" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                        </div>
                    </div>
                    ...
                    <div class='flex gap-x-4'>
                        ...
                        <label for="password" class="block text-sm/6 font-medium text-gray-100">Confirm Password*</label>
                            <div class="mt-2 relative">
                            <input onChange={(e)=>setpassc(e.target.value)} id="password" type={showPassword? "text":"password"} name="password" required autocomplete="password" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                            {showPassword? <svg
                                onClick={()=>setshowPassword(false)}
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 cursor-pointer">
                                <path
                    ...
                    <div className='flex flex-row'>
                        <button
                            ...
                            className={`w-[40px] h-6 flex items-center rounded-full p-1 transition-colors ${
                                terms ? "bg-indigo-500" : "bg-gray-400"
                            }`}
                            >
                                <div
                                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${
                                    terms ? "translate-x-[16px]" : "translate-x-0"
                                    }`}
                                ></div>
                        </button>
                        <label for="terms" class="block text-sm/6 font-medium text-gray-100">&nbsp;I hereby accept terms & conditions.</label>
                    </div>
                    <div>
